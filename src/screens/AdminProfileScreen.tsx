...
import { setAdminProfilePic } from '../store/slices';
...
import { AdminProfileScreenStyles as styles } from './styles/AdminProfileScreen.styles';
...
export default function AdminProfileScreen() {
  ...
  const adminCode = useSelector((state: RootState) => state.common.adminCode);
  ...
  const details = useMemo(
    () => [
      ...
      { label: 'Account ID', value: adminCode ?? adminId ?? 'Unavailable' },
    ],
    [adminCode, adminId, displayName],
  );
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      if (adminProfilePic) {
        ...
        dispatch(setAdminProfilePic(null));
      }

      const uploadedPath = await uploadProfilePic({
        file: {
      ...
      dispatch(setAdminProfilePic(uploadedPath ?? compressedImage.uri));
      Alert.alert(
